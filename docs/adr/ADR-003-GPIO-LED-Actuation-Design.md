...
| 類別 | GPIO 腳位（Board編號） | LED 顏色 | 限流電阻 |
|------|----------------------|---------|---------|
| 寶特瓶 | Pin 11（GPIO.0） | 綠色 | 220Ω |
| 鐵鋁罐 | Pin 13（GPIO.2） | 黃色 | 220Ω |
| 紙餐盒 | Pin 15（GPIO.3） | 藍色 | 220Ω |
| 塑膠袋 | Pin 19（GPIO.4） | 白色 | 220Ω |
| 鋁箔包 | Pin 21（GPIO.5） | 橙色 | 220Ω |
| 一般垃圾 | Pin 23（GPIO.6） | 紅色 | 220Ω |

### 電流計算
- LED 正向電壓（Vf）≈ 2.0V（紅/黃/綠）/ 3.0V（藍/白）
- GPIO 輸出電壓：3.3V
- 電流 = (3.3 - 2.0) / 220 ≈ 5.9mA（紅色）
- 電流 = (3.3 - 3.0) / 220 ≈ 1.4mA（藍/白，需改用 100Ω）
- 全亮 6 顆：約 30mA，遠低於 40mA/pin 限制 ✅

### 藍/白 LED 電阻修正
藍色與白色 LED Vf ≈ 3.0–3.2V，220Ω 電流過低（昏暗）。
**修正：藍色與白色 LED 改用 100Ω 限流電阻。**
